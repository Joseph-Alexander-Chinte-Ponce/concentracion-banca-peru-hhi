...
# ----------------------------------------------------------------------
# RUTAS DEL PROYECTO (relativas, numeral 2.4.4 de la consigna: prohibe
# rutas absolutas del computador del estudiante, tipo C:\Users\... o
# /content/drive/...). Este script vive en <proyecto>/codigo/, por lo
# que BASE_DIR sube un nivel para llegar a la raiz del proyecto.
# ----------------------------------------------------------------------
BASE_DIR = Path(__file__).resolve().parent.parent
CARPETA_CRUDOS = BASE_DIR / "datos_crudos"
CARPETA_LOGS = BASE_DIR
...
def procesar_variable(nombre_variable, dic_series, log_lineas):
    ...
    for idx, lote in enumerate(dividir_en_lotes(codigos, 10)):
        ...
        ruta_json = CARPETA_CRUDOS / f"{nombre_variable}_lote{idx}_{CODIGO_MATRICULA}.json"
...
def main():
    ...
    with open(CARPETA_LOGS / "log_ejecucion.txt", "a", encoding="utf-8") as f:
        f.write("\n".join(log_lineas) + "\n\n")

    print(f"\nListo. Revisa la carpeta {CARPETA_CRUDOS}: debe tener 8 archivos .json")
