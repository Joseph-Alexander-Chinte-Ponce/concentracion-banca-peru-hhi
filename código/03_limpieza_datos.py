...
Entrada:  <proyecto>/datos_crudos/*.json
Salida:   <proyecto>/datos_procesados/datos_procesados_2024200494M.csv
"""
...
SALIDA_CSV = CARPETA_PROCESADOS / f"datos_procesados_{CODIGO_MATRICULA}.csv"
...
def cargar_variable(nombre_variable):
    ...
    rutas = sorted(CARPETA_CRUDOS.glob(f"{nombre_variable}_lote*_{CODIGO_MATRICULA}.json"))
    if not rutas:
...
def cargar_tasas_sistema():
    ...
    """
    rutas = sorted(CARPETA_CRUDOS.glob(f"tasas_sistema_lote*_{CODIGO_MATRICULA}.json"))
    filas = []
    for ruta in rutas:
        with open(ruta, "r", encoding="utf-8") as f:
            data = json.load(f)
